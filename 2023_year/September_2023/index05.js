...
var garbageCollection = function (garbage, travel) {
    ...
    // initialize indexG, indexP, indexM to -1
    ...
    // loop through the every element of garbage
    for (let i = 0; i < garbage.length; i++) {

        // sum up the length of strings in Garbage to compute collection time
        time += garbage[i].length;

        // update the last seen index of 'G', 'P' and 'M' garbages
        if (garbage[i].includes('G')) indxG = i;
        if (garbage[i].includes('P')) indxP = i;
        if (garbage[i].includes('M')) indxM = i;
    }
    // increment the time with the travel time
